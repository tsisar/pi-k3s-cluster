...
9. `09-keycloak-realm-config.yaml` - ConfigMap з конфігурацією realm "rakia"
10. `10-keycloak-rakia-realm-job.yaml` - Job для створення realm "rakia"
...
```bash
# Застосувати Job для створення realm "rakia"
kubectl apply -f keycloak/10-keycloak-rakia-realm-job.yaml
...
### Зовнішній доступ
- URL: `http://auth.k3s-rakia.local`
- Admin Console: `http://auth.k3s-rakia.local/admin`
...
# Отримати токен
curl -X POST "http://localhost:8080/realms/rakia/protocol/openid-connect/token" \
