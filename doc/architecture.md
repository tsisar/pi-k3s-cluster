# Архітектура K3s кластера

Цей документ описує архітектуру та компоненти K3s кластера.
...
│                        Ingress Routes                           │
├─────────────────────────────────────────────────────────────────┤
│  auth.k3s-test.local     → Keycloak (8080)                     │
│  vault.k3s-test.local    → Vault (8200)                        │
│  grafana.k3s-test.local  → Grafana (3000)                      │
│  argocd.k3s-test.local   → ArgoCD (8080)                       │
│  api.k3s-test.local      → KrakenD (8080)                      │
│  prometheus.k3s-test.local → Prometheus (9090)                 │
└─────────────────────────────────────────────────────────────────┘
```
...
- **Worker nodes**: Додавання нових нодів як worker'ів
