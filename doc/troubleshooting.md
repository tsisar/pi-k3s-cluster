...
# Тест токену
curl -H "Authorization: Bearer <token>" https://api.k3s-test.local/api/v1/protected
```
...
# Перевірка JWK URL
curl https://auth.k3s-test.local/realms/master/protocol/openid-connect/certs
```
...
### Моніторинг
- **Grafana**: http://grafana.k3s-test.local
- **Prometheus**: http://prometheus.k3s-test.local
- **ArgoCD**: http://argocd.k3s-test.local
