...
resource "kubernetes_manifest" "keycloak_realm_config" {
  field_manager {
      ...
      "rakia-realm.json" = jsonencode({
            ...
            protocolMappers = [
              {
                ...
                config = {
                  "included.client.audience" = "krakend-test"
        ...
        roles = {
          realm = [
            {
              name = "krakend-user"
              description = "KrakenD user role"
            }
          ]
        }
        users = [
          {
            username = "krakend-service"
            enabled = true
            serviceAccountClientId = "krakend"
            credentials = [
              {
                type = "password"
                value = "krakend-service-password"
                temporary = false
              }
            ]
            realmRoles = ["krakend-user"]
          }
        ]
      })
    }
...
# Note: Keycloak realm import job managed via kubectl apply -f krakend/keycloak-rakia-realm-job.yaml
# Terraform provider has issues with Job resources, so we manage it separately